...
class BlocksCore(nn.Module):
    ...
    def __init__(self, ninp, nhid, num_blocks_in, num_blocks_out, topkval, step_att, do_gru, num_modules_read_input=2, use_higher=False, higher_separate_att = True):
        ...
        self.inp_att = MultiHeadAttention(n_head=self.inp_heads, d_model_read=self.block_size_out, d_model_write=self.ninp, d_model_out=self.att_out, d_k=32, d_v=d_v, num_blocks_read=num_blocks_out, num_blocks_write=num_modules_read_input,residual=False, topk=self.num_blocks_in+1, grad_sparse=False, skip_write=True, joined_heads_write=joined_heads_write)
